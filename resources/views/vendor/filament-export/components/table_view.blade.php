@php
    ...
    $shouldPrint =
        is_array($data) &&
        array_key_exists($tableViewName, $data) &&
        $data[$tableViewName] == 'print-' . $uniqueActionId;
...
<x-filament::modal id="preview-modal" width="7xl" display-classes="block" :dark-mode="config('filament.dark_mode')" x-init="$wire.$on('open-preview-modal-{{ $uniqueActionId }}', function() {
...
if ({{ $shouldPrint ? 'true' : 'false' }}) {
    ...
    <x-slot name="footer">
        @php
            $footerActions = collect($getFooterActions())
                ->filter(function ($action) {
                    return method_exists($action, 'getName') ? $action->getName() !== 'print' : true;
                })
                ->all();
        @endphp
