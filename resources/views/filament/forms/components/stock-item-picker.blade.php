...
<x-dynamic-component :component="$getFieldWrapperView()" :field="$field">
    <div x-data="{
            ...
            <div x-show="selectedId && getSelectedStockItem()" x-transition:enter="transition ease-out duration-200"
                ...
                <div class="flex items-start gap-4">
                    <img :src="getSelectedStockItem()?.image_path || '{{ $fallbackImage }}'"
                        :alt="getSelectedStockItem()?.product_name" class="h-20 w-20 rounded-lg object-cover"
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-sm text-gray-600" x-text="'Kategori: ' + getSelectedStockItem()?.category_name">
                        </p>
                        <div class="mt-2">
                            <p class="text-xs font-medium text-gray-700 mb-1">Uygulanabilir Alanlar:</p>
                            <div class="flex flex-wrap gap-1">
                                <template x-for="(part, index) in getSelectedStockItem()?.available_parts_labels || []"
                                    :key="index">
                                    <span
                        ...
                        @if (config('app.debug'))
                            <div class="mt-2 pt-2 border-t border-amber-300">
                                <div><strong>SQL Sorgusu:</strong> <code
                                        class="text-xs">{{ $debugInfo['querySql'] }}</code></div>
                                <div class="mt-1"><strong>Bindings:</strong> <code
                                        class="text-xs">{{ json_encode($debugInfo['queryBindings']) }}</code></div>
                            </div>
                        @endif
                    </div>
                </details>
                ...
                <div class="overflow-y-auto p-6" style="max-height: calc(90vh - 180px);">
                        ...
                        <template x-for="item in filteredStockItems" :key="item.id">
                            <div @click="!isDisabled && selectStockItem(item.id)"
                                ...
                                <div class="mt-2">
                                    ...
                                    <div class="flex flex-wrap gap-1">
                                        <template x-for="(part, index) in item.available_parts_labels || []"
                    ...
                    <div x-show="stockItems.length === 0" class="text-center text-gray-500 py-8 space-y-2">
                        ...
                        <details class="text-xs text-gray-600 inline-block">
                            <summary class="cursor-pointer font-medium hover:text-gray-700">🔍 Debug Bilgileri
                            </summary>
                            <div
                                class="mt-2 text-left space-y-1 font-mono bg-gray-100 p-2 rounded border border-gray-300 max-w-md mx-auto">
                                <div><strong>Uygulama Alanları:</strong>
                                    <span x-text="JSON.stringify(@js($appliedParts) || [])"></span>
                                ...
                                @if (config('app.debug'))
                                    <div class="mt-2 pt-2 border-t border-gray-300">
                                        ...
                                        <div class="mt-1"><strong>Bindings:</strong> <code
                                                class="text-xs">{{ json_encode($debugInfo['queryBindings']) }}</code>
                                        </div>
                                    </div>
                                @endif
                            </div>
        ...
        <!-- QR Scanner -->
        <div x-show="isScanning" x-cloak class="fixed z-50 inset-0 flex items-center justify-center bg-black/50 p-4"
            style="display: none;">
            <div class="relative w-full max-w-md bg-white rounded-lg shadow-xl p-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <button @click="stopQRScan()" type="button" class="text-gray-400 hover:text-gray-600">
                        <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M6 18L18 6M6 6l12 12"></path>
                        </svg>
